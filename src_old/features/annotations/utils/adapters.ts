  ...
  return predictions.map((pred: any, index: number) => {
    ...
    if (pred.segmentation && Array.isArray(pred.segmentation)) {
      ...
      if (pred.keypoints && Array.isArray(pred.keypoints)) {
        ...
        console.log(`  Found ${keypoints.length} keypoints for pose detection`);
      }
      ...
      const annotation: UIAnnotation = {
        ...
        // ✅ add keypoints for crown/pose detection
        keypoints,
